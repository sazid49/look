...
@section('title', __('labels.offer_field.edit_field'))
...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box1 mb-2 d-sm-flex align-items-center justify-content-between">
                <div class="col-md-6">
                    <h4 class="mb-sm-0 font-size-18">{{ __('labels.offer_field.edit_field') }}</h4>
                ...
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a
                                href="{{ route('admin.offer_field.index') }}">{{ __('labels.offer_field.management') }}</a>
                        </li>
                        <li class="breadcrumb-item active">{{ __('labels.offer_field.edit_field') }}</li>
                    </ol>
                </div>
    ...
    <!-- end page title -->
    <x-forms.patch :action="route('admin.offer_field.update',$offerFieldGroup)" enctype="multipart/form-data" class="custom-validation outer-repeater1" novalidate>
        <x-backend.card>
            <x-slot name="body">
                <div data-repeater-list="outer-group" class="outer">
                    <div data-repeater-item class="outer">
                        <div class="row">
                            @foreach($offerFieldGroup->offerFields as $field)
                                <div class="col-lg-3">
                                    <div class="mb-3">
                                        <label for="label" class="form-label">{{ __('labels.offer_field.label_'.$field->language) }}<span class="error">*</span></label>
                                        <input type="text" name="label[{{$field->language}}]" class="form-control" value="{{ old('label_'.$field->language) ?? $field->label }}" {{ $field->language == 'de' ? 'required' : '' }} />
                                    </div>
                                </div>
                            @endforeach
                        </div>
                            ...
                            <div class="col-lg-4">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="type" id="type" class="form-control" required>
                                        ...
                                        <option value="text" {{ $offerFieldGroup->type == 'text' ? 'selected' : '' }}>Text</option>
                                        <option value="textarea" {{ $offerFieldGroup->type == 'textarea' ? 'selected' : '' }}>
                                            Textarea</option>
                                        <option value="dropdown" {{ $offerFieldGroup->type == 'dropdown' ? 'selected' : '' }}>
                                            DropDown</option>
                                        <option value="checkbox" {{ $offerFieldGroup->type == 'checkbox' ? 'selected' : '' }}>
                                            Checkbox</option>
                                        <option value="radio" {{ $offerFieldGroup->type == 'radio' ? 'selected' : '' }}>Radio Button</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-lg-4">
                                    ...
                                    <select name="category_id" id="category_id" class="form-control" required>
                                        ...
                                        @foreach ($category as $item)
                                            <option value="{{ $item->id }}"
                                                {{ $offerFieldGroup->category_id == $item->id ? 'selected' : '' }}>
                                                {{ $item->value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <div class="mb-2 mt-2">
                                        <label for="is_required">
                                            <input type="radio" name="is_required" value="1"
                                                {{ $offerFieldGroup->is_required == '1' ? 'checked' : '' }} />
                                            {{ __('labels.general.yes') }} &nbsp;&nbsp;
                                        </label>
                                        <label for="is_required">
                                            <input type="radio" name="is_required" value="0"
                                                   {{ $offerFieldGroup->is_required == '0' ? 'checked' : '' }}
                        ...
                        <div class="row">
                            @if(in_array($offerFieldGroup->type,['dropdown','checkbox','radio']))
                                <div class="inner-repeater option_div mb-3">
                                    <div data-repeater-list="inner-group" class="inner mb-3">
                                        <label>{{ __('labels.offer_field.option') }}</label>
                                        <div class="prising_div">
                                            @php $optionCount = count(json_decode($field->option)) @endphp
                                            @for($i=0;$i<$optionCount;$i++)
                                                <div data-repeater-item class="inner mb-3 row item_title">
                                                    @foreach($offerFieldGroup->offerFields as $field)
                                                        <div class="col-md-2">
                                                            <input type="text"
                                                                   name="option[{{$field->language}}][]"
                                                                   data-role="tagsinput"
                                                                   class="form-control option"
                                                                   value="{{ json_decode($field->option)[$i] }}"
...
{{--                            </div>--}}
            ...
            <x-slot name="footer">
                <button class="btn btn-primary float-right" type="submit">{{ __('labels.offer_field.update_field') }}</button>
            </x-slot>
        </x-backend.card>
    </x-forms.patch>
@endsection
