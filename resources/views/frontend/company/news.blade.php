                                                        ...
                                                        <div class="col-md-12">
                                                            <div class="card flex-md-row mb-4 box-shadow h-md-250">
                                                                ...
                                                                <div class="card-body d-flex flex-column align-items-start">
                                                                    <h4 class="mb-0">
                                                                        <a class="text-dark"
                                                                           href="{{ route('frontend.company.news_detail',[$company,$news['id'],$company->slug]) }}">{{ $news->title }}</a>
                                                                    </h4>
                                                                    <div class="mb-1 text-muted">{{ $news->author }}</div>
                                                                        <?php $story_desc = substr ( $news->description, 0, 150 );
                                                                        $story_desc = substr ( $story_desc, 0, strrpos ( $story_desc, ' ' ) );
                                                                        echo strip_tags ( htmlspecialchars_decode ( $story_desc ) ) . '...'; ?>
                                                                    <a href="{{route('frontend.company.news_detail',[$company,$news['id'],$company->slug])}}">{{__('labels.general.read_more')}}</a>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    @endif
