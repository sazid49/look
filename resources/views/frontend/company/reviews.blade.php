    ...
    <script type="application/ld+json">
		{
		  ...
		  "itemReviewed": {
			"@type": "Company",
			"image": "{{ url('storage/' . $company->company_logo) }}",
			"name": "{{$company->company_name}}",
			"servesCuisine": "{{ $company->keyword }}",
			"priceRange": "$$$",
			"telephone": "{{$company->phone_1 }}",
			"address" :{
			  "@type": "PostalAddress",
      ...
      "reviewRating": {
        ...
        "ratingValue": "{{ $company->average_rating }}"
		  ...
		  "name": "{{ $company->description_en }}",
		  "author": {
			"@type": "Person",
			"name": "{{ $company->claimed_user->name }}"
		  }{{-- ,
		  "publisher": {
			"@type": "Organization",
                    ...
                    <div id="listing-overview" class="listing-section">
                        <div class="card mb-4">
                            <div class="card-header subtitle border-bottom pb-3 d-flex align-items-center">
                                <h5> @lang('labels.review.heading_title', ['company' => $company->company_name])</h5>
                            </div>
                            <div id="listing-reviews">
                                <!-- Rating Overview -->
                                <div class="rating-overview">
                                    <div class="rating-overview-box">
                                        <span class="rating-overview-box-total">{{ $company->average_rating }}</span>
                                    ...
                                    <div class="rating-bars">
                                        @if (!$company->category->reviewcategorycriteria->isEmpty())
                                            @foreach ($company->category->reviewcategorycriteria as $reviewcategorycriteria)
                                                @php
                                                    $category_review = $reviewcategorycriteria->review_criteria->allreview->where('listing_id', $company->id)->sum('rating');
                                                    $total_review = $reviewcategorycriteria->review_criteria->allreview->where('listing_id', $company->id)->count() * 5;
                                                    $per = 0;
                                                    if ($total_review != 0) {
                                ...
                                {{--                                @php--}}
                                {{--                                    $listing_reviews = $company->CompanyReview->sortByDesc('id');--}}
                                {{--                                @endphp--}}
                                @if (count($listing_reviews))
                                    ...
                                    <section class="comments ">
                                        <ul class="mb-0 listing-reviews">
                                            @foreach ($listing_reviews as $key_id => $listing_review)
                                                <li>
                                                    <div class="col-md-12">
                                                        <div class="col-md-12">
                                                            <div class="avatar">
                                                                @if ($listing_review->user_id != '')
                                                                    <img src="{{ url('storage/' . $listing_review->User->avatar) }}?d=mm&amp;s=70"
                                                                         alt="{{ ucfirst($listing_review->User->name) }}"/>
                                                                @else
                                                            ...
                                                            <div class="comment-content">
                                                                ...
                                                                <div class="comment-by">
                                                                    ...
                                                                    <div class="date d-flex">
                                                                        <span>{{ $listing_review->postcode }}</span>&nbsp;
                                                                        <span>{{ $listing_review->city }}</span>&nbsp;

                                                                        <span class="mx-2 text-secondary"
                                                                              style="opacity: 0.6">|</span>
                                                                        <span>{{ $listing_review->created_at->format('d.m.Y') }}</span>
                                                                    </div>
                                                                    @php
                                                                        $average_rating = 0;
                                                                    ...
                                                                    <div class="col-md-12 mt-3">
                                                                        <div class="form-group row">
                                                                            <label class="control-label col-md-2">
                                                                                @if ($listing_reviews[$key_id]->company->company_logo != '')
                                                                                    <img class="rounded-circle"
                                                                                         src="{{ url('storage/' . $listing_reviews[$key_id]->company->company_logo) }}"
                                                                                         alt="{{ ucfirst($listing_reviews[$key_id]->company->company_name) }}"
                                                                            ...
                                                                            <label
                                                                                    class="control-label col-md-10 small">{{ $listing_reviews[$key_id]->ReviewReply->replay_message }}</label><br>
                                                                        </div>
                                                                    </div>
                                                                @endif
                                                            </div>
                                                        ...
                                                        <div class="add-sub-rating d-flex">
                                                            ...
                                                            <div class="">
                                                                ...
                                                                <div class="sub-rating-stars">
                                                                    ...
                                                                    <span
                                                                            for="review[{{ $reviewcategorycriteria->review_criteria->id }}]"></span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div class="row">
                                                <div class="col">
                                                    <div class="form-group">
                                                        ...
                                                        <label for="postcode"
                                                               class="col-form-label">@lang('labels.review.postcode')</label>
                                                        <div class="input-group">
                                                ...
                                                <div class="col-md-2">
                                                    @if (!empty($logged_in_user->avatar))
                                                        <img class="rounded-circle1 header-profile-user1" width="100px"
                                                             src="{{ url('storage/' . $logged_in_user->avatar) }}"
                                            ...
                                            <div class="col">
                                                <div class="form-group">
                                                    <label for="review_message"
                                                           class="col-form-label">@lang('labels.review.message')</label>
            ...
            var showAllErrorMessages = function () {
                ...
                var invalidFields = form.find(":invalid").each(function (index, node) {
                    ...
                    errorList
                        ...
                        .html(
                            "<li style='color:#dc3545'><span>Error:</span> Please select start and enter comment</li>"
                        );
                });
            };
